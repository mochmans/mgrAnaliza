...
HistogramGroups loadkStarHistograms(const char* fileName)

{
    ...
    if(corDir) {    
        ...
        if(omegakStarNames[0] != "") {
            for (int i = 0; i < omegakStarNames.size(); i++) {
                TH1F* hist = (TH1F*)corDir->Get(omegakStarNames[i]);
                hists.omegaCor.push_back(hist);
            }
        }
        
        if(antiOmegakStarNames[0] != "") {
            for (int i = 0; i < antiOmegakStarNames.size(); i++) {
                TH1F* hist = (TH1F*)corDir->Get(antiOmegakStarNames[i]);
                hists.antiOmegaCor.push_back(hist);
            }
        }   
    }
